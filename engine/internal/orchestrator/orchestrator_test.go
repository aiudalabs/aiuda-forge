...
// setPlanningMode sets a project's execution_unit + planning_mode (the ceremony
// tests). Merges into any existing override so merge_mode is preserved.
func (f *fakeControlPlane) setPlanningMode(projectID, executionUnit, planningMode string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.projectModes == nil {
		f.projectModes = map[string]projectMode{}
	...
	m := f.projectModes[projectID]
	m.executionUnit = executionUnit
	m.planningMode = planningMode
	f.projectModes[projectID] = m
}

// setReviewMode sets a project's execution_unit + review_mode (the sprint-review
// ceremony tests). Merges into any existing override so other settings are preserved.
func (f *fakeControlPlane) setReviewMode(projectID, executionUnit, reviewMode string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.projectModes == nil {
		f.projectModes = map[string]projectMode{}
