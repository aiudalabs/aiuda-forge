...
func (f *fakeControlPlane) ProjectSettings(_ context.Context, projectID string) (string, string, string, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if m, ok := f.projectModes[projectID]; ok {
		return m.executionUnit, m.mergeMode, m.planningMode, nil
	}
	unit := f.execUnit
	if unit == "" {
		unit = "story"
