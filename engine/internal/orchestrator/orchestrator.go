...
type ControlPlane interface {
	...
	// "auto"|"ceremony") from GET /projects/{id}/settings (audit A2). The native
	// scheduler reads it per-project each cycle so two projects can run under
	// different modes in the same cycle. An empty projectID resolves to the default
	// project's settings.
	ProjectSettings(ctx context.Context, projectID string) (executionUnit, mergeMode, planningMode string, err error)
