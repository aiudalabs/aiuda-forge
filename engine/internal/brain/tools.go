...
var registry = map[string]toolDef{
	...
	"answer_step": {
		Tool: Tool{Name: "answer_step", Description: "Answer a run's awaiting human-gate open questions WITHOUT rejecting it. Re-runs the phase incorporating the answers into the EXISTING doc (no regeneration), then re-parks the gate for approval. Does NOT count as a rejection (on_fail budget untouched). MUTATING — proposed for human approval.", InputSchema: obj(map[string]any{"run_id": strp("run id"), "step": strp("step id"), "text": strp("the answers to the phase's open questions")}, "run_id", "step", "text")},
		Kind: Mutating, MinRole: "editor",
		Run: func(ops ControlOps, _ string, input json.RawMessage) (string, error) {
